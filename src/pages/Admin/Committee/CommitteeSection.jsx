...
import { useCommittee } from "../../../Hooks/Hooks";
...
export default function CommitteeSection() {
  ...
  const handleDelete = async (id) => {
    try {
      const res = await fetch(`http://localhost:5000/api/committees/${id}`, { method: "DELETE" });
  ...
  const baseRows = activeTab === 0
    ? rows.filter(r => r.role === "general")
    : rows.filter(r => r.role !== "general");
  console.log(rows, ' rows after tab filter');
  ...
  // console.log(filtered, 'filtered members');
  ...
  return (
    <div className="h-full flex flex-col bg-[#f0ede8] font-['Jost']">
      ...
      {confirm && (
        <ConfirmDialog
          message="This member will be permanently deleted. Are you sure?"
      ...
      <div className="bg-white border-b border-[#e8e2d9] px-6 py-4 shrink-0">
        ...
        <div className="flex gap-1 mt-4">
          <button
            ...
            className={`px-4 py-1.5 text-[10px] font-medium tracking-[0.14em] uppercase rounded-sm transition-colors
              ${activeTab === 0 ? "bg-[#2b3232] text-[#e8e2d9]" : "text-[#9aabab] hover:text-[#2b3232] hover:bg-[#f5f2ee]"}`}
          >
            General Members
          </button>
          <button
            ...
            className={`px-4 py-1.5 text-[10px] font-medium tracking-[0.14em] uppercase rounded-sm transition-colors
              ${activeTab === 1 ? "bg-[#2b3232] text-[#e8e2d9]" : "text-[#9aabab] hover:text-[#2b3232] hover:bg-[#f5f2ee]"}`}
          >
            Executive Members
          </button>
        </div>
          ...
          {isLoading ? (
            ...
            <div className="flex-1 flex items-center justify-center">
              <div className="flex flex-col items-center gap-3">
                ...
                <p className="text-xs text-red-500">{fetchError.message || "Failed to load members"}</p>
            ...
            <EmptyState label="members" readOnly={false} onAdd={() => setModal({})} />
          ) : (
            <>
              {/* Header */}
              <div className="shrink-0 grid border-b border-[#f0ede8] bg-[#f8f6f3]" style={{ gridTemplateColumns: "2fr 1.5fr 1.5fr 120px" }}>
                ...
                <div className="px-4 py-3 text-[9px] font-medium tracking-[0.2em] uppercase text-[#9aabab]">Role</div>
              ...
              <div className="flex-1 overflow-y-auto divide-y divide-[#f0ede8]">
                {filtered.map((row, idx) => (
                  <div key={row._id || idx} className="grid items-center hover:bg-[#faf9f7] transition-colors group" style={{ gridTemplateColumns: "2fr 1.5fr 1.5fr 120px" }}>
                    ...
                    <div className="px-4 py-3"><Badge value={row.role} /></div>
...
function MemberModal({ record, onClose, onSaved }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = isEdit ? `http://localhost:5000/api/committees/${record._id}` : `http://localhost:5000/api/committees`;
      ...
      setErrors({ _global: error.message || "Failed to save member. Please try again." });
    }
  };
    ...
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-sm shadow-xl w-full max-w-md overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#f0ede8]">
          <div>
            <p className="text-[9px] font-medium tracking-[0.22em] uppercase text-[#a0724a]">Committee</p>
            <h3 className="font-['Cormorant_Garamond'] text-[#2b3232] text-xl font-medium">{isEdit ? "Edit Member" : "Add Member"}</h3>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4 max-h-[65vh] overflow-y-auto">
          ...
          <div>
            <label className="block text-[10px] font-medium tracking-[0.16em] uppercase text-[#3d5454] mb-1.5">Role *</label>
          ...
          <div>
            <label className="block text-[10px] font-medium tracking-[0.16em] uppercase text-[#3d5454] mb-1.5">Batch</label>
            <input type="text" className={inputCls("batch")} placeholder="Batch" value={form.batch} onChange={e => setForm({ ...form, batch: e.target.value })} />
          </div>

          <div>
            <label className="block text-[10px] font-medium tracking-[0.16em] uppercase text-[#3d5454] mb-1.5">Photo URL</label>
            <input type="url" className={inputCls("photoUrl")} placeholder="Photo URL" value={form.photoUrl} onChange={e => setForm({ ...form, photoUrl: e.target.value })} />
          </div>
        </form>
