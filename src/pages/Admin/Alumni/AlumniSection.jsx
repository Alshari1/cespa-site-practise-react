...
export default function AlumniSection() {
  ...
  const handleDelete = async (id) => {
    try {
      const res = await fetch(`http://localhost:5000/api/alumni/${id}`, { method: "DELETE" });
  ...
  const handleSaved = (isEdit) => {
    setModal(null);
    showToast(isEdit ? "Alumni updated" : "Alumni added");
  };
  ...
  return (
    <div className="h-full flex flex-col bg-[#f0ede8] font-['Jost']">
      ...
      {confirm && (
        <ConfirmDialog
          message="This alumni record will be permanently deleted. Are you sure?"
          ...
          {isLoading ? (
            ...
            <div className="flex-1 flex items-center justify-center">
              <div className="flex flex-col items-center gap-3">
                ...
                <p className="text-xs text-red-500">{fetchError.message || "Failed to load alumni"}</p>
            ...
            <EmptyState label="alumni" readOnly={false} onAdd={() => setModal({})} />
          ) : (
            <>
              {/* Header */}
              <div className="shrink-0 grid border-b border-[#f0ede8] bg-[#f8f6f3]" style={{ gridTemplateColumns: "2fr 1.5fr 1.5fr 120px" }}>
                ...
                <div className="px-4 py-3 text-[9px] font-medium tracking-[0.2em] uppercase text-[#9aabab]">Batch</div>
              ...
              <div className="flex-1 overflow-y-auto divide-y divide-[#f0ede8]">
                {filtered.map((row, idx) => (
                  <div key={row._id || idx} className="grid items-center hover:bg-[#faf9f7] transition-colors group" style={{ gridTemplateColumns: "2fr 1.5fr 1.5fr 120px" }}>
                    ...
                    <div className="px-4 py-3 text-xs text-[#4a5555] truncate">{row.batch || "—"}</div>
...
function AlumniModal({ record, onClose, onSaved }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = isEdit ? `http://localhost:5000/api/alumni/${record._id}` : `http://localhost:5000/api/alumni`;
      ...
      setErrors({ _global: error.message || "Failed to save alumni. Please try again." });
    }
  };
    ...
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-sm shadow-xl w-full max-w-md overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#f0ede8]">
          <div>
            <p className="text-[9px] font-medium tracking-[0.22em] uppercase text-[#a0724a]">Alumni</p>
            <h3 className="font-['Cormorant_Garamond'] text-[#2b3232] text-xl font-medium">{isEdit ? "Edit Alumni" : "Add Alumni"}</h3>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4 max-h-[65vh] overflow-y-auto">
          ...
          <div>
            ...
            {errors.batch && <p className="text-[10px] text-red-400 mt-1">{errors.batch}</p>}
          </div>
          ...
          <div>
            <label className="block text-[10px] font-medium tracking-[0.16em] uppercase text-[#3d5454] mb-1.5">Company</label>
            <input type="text" className={inputCls("company")} placeholder="Company" value={form.company} onChange={e => setForm({...form, company: e.target.value})} />
          </div>

          <div>
            <label className="block text-[10px] font-medium tracking-[0.16em] uppercase text-[#3d5454] mb-1.5">LinkedIn URL</label>
            <input type="url" className={inputCls("linkedin")} placeholder="LinkedIn URL" value={form.linkedin} onChange={e => setForm({...form, linkedin: e.target.value})} />
          </div>
        </form>
