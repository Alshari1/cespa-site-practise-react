...
export default function NoticesSection() {
  ...
  const handleSaved = (isEdit) => {
    setModal(null);
    showToast(isEdit ? "Notice updated" : "Notice added");
  };
  ...
  return (
    <div className="h-full flex flex-col bg-[#f0ede8] font-['Jost']">
      ...
      {confirm && (
        <ConfirmDialog
          message="This notice will be permanently deleted. Are you sure?"
          ...
          {isLoading ? (
            ...
            <div className="flex-1 flex items-center justify-center">
              <div className="flex flex-col items-center gap-3">
                ...
                <p className="text-xs text-red-500">{fetchError.message || "Failed to load notices"}</p>
            ...
            <EmptyState label="notices" readOnly={false} onAdd={() => setModal({})} />
          ) : (
            <>
              {/* Header */}
              <div className="shrink-0 grid border-b border-[#f0ede8] bg-[#f8f6f3]" style={{ gridTemplateColumns: "2fr 1.5fr 1.5fr 120px" }}>
                ...
                <div className="px-4 py-3 text-[9px] font-medium tracking-[0.2em] uppercase text-[#9aabab]">Date</div>
                <div className="px-4 py-3 text-[9px] font-medium tracking-[0.2em] uppercase text-[#9aabab]">Category</div>
              ...
              <div className="flex-1 overflow-y-auto divide-y divide-[#f0ede8]">
                {filtered.map((row, idx) => (
                  <div key={row._id || idx} className="grid items-center hover:bg-[#faf9f7] transition-colors group" style={{ gridTemplateColumns: "2fr 1.5fr 1.5fr 120px" }}>
                    ...
                    <div className="px-4 py-3 text-xs text-[#4a5555] truncate">{row.date ? new Date(row.date).toLocaleDateString() : "—"}</div>
                    <div className="px-4 py-3"><Badge value={row.category} /></div>
...
function NoticeModal({ record, onClose, onSaved }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = isEdit ? `http://localhost:5000/api/notices/${record._id}` : `http://localhost:5000/api/notices`;
      ...
      setErrors({ _global: error.message || "Failed to save notice. Please try again." });
    }
  };
    ...
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-sm shadow-xl w-full max-w-md overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#f0ede8]">
          <div>
            <p className="text-[9px] font-medium tracking-[0.22em] uppercase text-[#a0724a]">Notices</p>
            <h3 className="font-['Cormorant_Garamond'] text-[#2b3232] text-xl font-medium">{isEdit ? "Edit Notice" : "Add Notice"}</h3>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4 max-h-[65vh] overflow-y-auto">
          ...
          <div>
            ...
            <input type="text" className={inputCls("title")} placeholder="Title" value={form.title} onChange={e => setForm({...form, title: e.target.value})} />
          ...
          <div>
            ...
            <input type="date" className={inputCls("date")} value={form.date} onChange={e => setForm({...form, date: e.target.value})} />
            {errors.date && <p className="text-[10px] text-red-400 mt-1">{errors.date}</p>}
          </div>

          <div>
            <label className="block text-[10px] font-medium tracking-[0.16em] uppercase text-[#3d5454] mb-1.5">Content</label>
            <textarea rows={3} className={inputCls("body")} placeholder="Notice Content" value={form.body} onChange={e => setForm({...form, body: e.target.value})} />
          </div>
        </form>
